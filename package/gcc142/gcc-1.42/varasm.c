     ...
     enum machine_mode mode;
{
  ...
  if (u.i[0] == CONST_DOUBLE_LOW (dconst0_rtx)
      && u.i[1] == CONST_DOUBLE_HIGH (dconst0_rtx))
  ...
  for (r = real_constant_chain; r; r = CONST_DOUBLE_CHAIN (r))
    if (u.i[0] == CONST_DOUBLE_LOW (r) && u.i[1] == CONST_DOUBLE_HIGH (r)
	&& GET_MODE (r) == mode)
      return r;
  ...
  PUT_MODE (r, mode);
  CONST_DOUBLE_FROM_REAL_VALUE (r, u);
     ...
     rtx x;
{
  ...
  if (found == 0)
      ...
      if (GET_CODE (x) == CONST_DOUBLE)
	{
	  ...
	  REAL_VALUE_FROM_CONST_DOUBLE (u, x);
	  switch (mode)
	    {
