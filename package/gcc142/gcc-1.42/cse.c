...
new_basic_block ()
{
  ...
  register int int_vecsize = max_reg * sizeof (int);
  next_qty = max_reg;

  bzero (reg_rtx, vecsize);
  bzero (reg_tick, int_vecsize);

  bcopy (all_minus_one, reg_in_table, int_vecsize);
  bcopy (all_minus_one, reg_next_eqv, int_vecsize);
  bcopy (all_minus_one, reg_prev_eqv, int_vecsize);
  bcopy (consec_ints, reg_qty, int_vecsize);
     ...
     int copyflag;
{
  ...
  if (GET_RTX_LENGTH (code) == 1)
	       ...
	       && GET_MODE_CLASS (GET_MODE (x)) == MODE_FLOAT)
	{
	  ...
	  REAL_VALUE_FROM_CONST_DOUBLE (u, const_arg0);
	  arg0 = u.d;
     ...
     rtx x;
{
  if (GET_CODE (x) == COMPARE)
	  ...
	  && GET_MODE_CLASS (GET_MODE (y0)) == MODE_FLOAT)
	{
	  ...
	  set_float_handler (handler);
	  REAL_VALUE_FROM_CONST_DOUBLE (u0, y0);
	  REAL_VALUE_FROM_CONST_DOUBLE (u1, y1);
